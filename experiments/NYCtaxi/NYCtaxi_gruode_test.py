...
def train_gruode(simulation_name,params_dict,device, train_idx, val_idx, test_idx, epoch_max=40):
    ...
    dl   = DataLoader(dataset=data_train, collate_fn=data_utils.custom_collate_fn, shuffle=True, batch_size=28,num_workers=2)
    ...
    with torch.no_grad():
        ...
        for i, b in enumerate(dl_test):
            ...
            if params_dict["lambda"]==0:
                ...
                corr_val_loss = data_utils.compute_corr(X_val, m, M_val)
                print("=== Raw-scale prediction vs ground truth ===")
                for i in range(min(5, X_raw.shape[0])):
                    print(f"Sample {i}: GT={X_raw[i].cpu().numpy()}   Pred={m_raw[i].cpu().numpy()}")
...
if __name__ =="__main__":
    ...
    params_dict["csv_file_path"] = "../../gru_ode_bayes/datasets/NYCtaxi/NYCtaxi_2017-01_irregular_region_1_80_norm.csv" 
    ...
    state_dict = torch.load('/home/zhangdi24/gru_ode_bayes/experiments/trained_models/small_climate_MAX.pt', map_location=device)
