...
class ODE_Dataset(Dataset):
    ...
    def __init__(self, csv_file=None, cov_file=None, label_file=None, panda_df=None, cov_df=None, label_df=None, root_dir="./", t_mult=1.0, idx=None, jitter_time=0, validation = False, val_options = None, A_file=None):
        ...
        if A_file is not None:
            ...
            print('A_all is not None!')
        else: 
            self.A_all = None
        adj_dict = {
            1: np.load("/home/zhangdi24/LLM4TS/models/ode/preproc/NYC/NYCtaxi/data_accu/train/adjacency_matrix_80x80.npy"),
            2: np.load("/home/zhangdi24/LLM4TS/models/ode/preproc/NYC/NYCbike/data_accu/train/adjacency_matrix_80x80.npy"),
            3: np.load("/home/zhangdi24/LLM4TS/models/ode/preproc/NYC/NYCcrime/data/train/adjacency_matrix_80x80.npy")
        }
        ...
        if self.validation:
            ...
            elif val_options.get("val_step") is not None:
                step = val_options["val_step"]
                self.df_after = self.df_after.groupby("ID").apply(lambda group: group.iloc[::step]).reset_index(drop=True)
            else:
                self.df_after  = self.df_after.groupby("ID").head(val_options["max_val_samples"]).copy()
    ...
    def __getitem__(self, idx):
        ...
        if self.A_all is not None:
            A = self.A_all
        else:
            ds_id = int(subset["dataset_id"].iloc[0])
            A = self.adj_dict[ds_id]
...
def custom_collate_fn(batch):
    ...
    pat_idx   = [b["idx"] for b in batch] # 每条序列的样本ID
    ...
    cov_list = [b["cov"] for b in batch]
    df_cov = torch.from_numpy(np.stack(cov_list, axis=0)).float()

    y_list = [b["y"] for b in batch]
    labels  = torch.from_numpy(np.stack(y_list, axis=0)).float()
    ...
    # interval_tensor = torch.tensor(df["interval"].values, dtype=torch.float32).view(-1, 1)
    ...
    if batch[0]['val_samples'] is not None:
        ...
        # interval_val_tensor = torch.tensor(df_after["interval"].values,
    ...
    # res["interval_val"] = interval_val_tensor
    return res
# res记录的是将一个 batch 内所有样本的所有时间点整合成一条按时间排序的“全局事件序列”，并且记录了：
# (1)每个时间点下有多少事件
# (2)每个事件的观测值（X/M/interval）
# (3)每个事件来自哪个样本（obs_idx）
# 用于让 GRU-ODE 在全局时间轴上执行 ODE 演化 + GRU 更新。
...
# class Graph_ODE_Dataset(Dataset):
#     """
#     Dataset class for ODE type of data. With 2 values.
#     Can be fed with either a csv file containg the dataframe or directly with a panda dataframe.
#     One can further provide samples idx that will be used (for training / validation split purposes.)
...
#         #Create Dummy covariates and labels if they are not fed.
#         if self.cov_df is None:
#             num_unique = np.zeros(self.df["sub_ID"].nunique())
#             self.cov_df = pd.DataFrame({"sub_ID":self.df["sub_ID"].unique(),"Cov": num_unique})
#         if self.label_df is None:
#             num_unique = np.zeros(self.df["sub_ID"].nunique())
#             self.label_df = pd.DataFrame({"sub_ID":self.df["sub_ID"].unique(),"label": num_unique})
...
#         assert self.cov_df.shape[0]==self.df["sub_ID"].nunique()

#         self.variable_num = sum([c.startswith("Value") for c in self.df.columns]) #number of variables in the dataset
#         self.cov_dim = self.cov_df.shape[1]-1

#         self.cov_df = self.cov_df.astype(np.float32)
#         self.cov_df.set_index("sub_ID", inplace=True)

#         self.label_df.set_index("sub_ID",inplace=True)
...
#         sub_ids = subset['sub_ID'].unique()
#         covs = self.cov_df.loc[sub_ids].values
#         tag  = self.label_df.loc[sub_ids].astype(np.float32).values
#         if self.validation :
#             val_samples = self.df_after.loc[self.df_after["ID"]==idx]
#         else:
#             val_samples = None
...
#     res_list = []
#     for sub_seq in batch:
#         df_seq = sub_seq['path']
#         df_cov = sub_seq['cov']
#         labels = sub_seq['y'] # 注：labels和cov的顺序可能和idx2batch不一致
#         df_sub = []
#         for i, (sub_id, df_sub_ID_single) in enumerate(df_seq.groupby('sub_ID')):
#             df_sub_ID_single = df_sub_ID_single.drop(columns=["ID"])
#             df_sub.append(df_sub_ID_single)
        
#         idx2batch = pd.Series(np.arange(len(df_sub)), index = [b["sub_ID"].iloc[0] for b in df_sub])

#         pat_idx   = [b["sub_ID"].iloc[0] for b in df_sub] # 每条序列的样本ID
#         df        = pd.concat([b for b in df_sub],axis=0)
#         df.sort_values(by=["Time"], inplace=True)
#         df.reset_index(drop=True, inplace=True)

#         batch_ids     = idx2batch[df['sub_ID'].values].values
...
#         if batch[0]['val_samples'] is not None:
#             df_after = sub_seq['val_samples']
#             df_after.sort_values(by=["sub_ID","Time"], inplace=True)
#             value_cols_val = [c.startswith("Value") for c in df_after.columns]
#             mask_cols_val  = [c.startswith("Mask") for c in df_after.columns]
#             X_val = torch.tensor(df_after.iloc[:,value_cols_val].values)
#             M_val = torch.tensor(df_after.iloc[:,mask_cols_val].values)
#             times_val = df_after["Time"].values
#             index_val = idx2batch[df_after["sub_ID"].values].values
#             # interval_val_tensor = torch.tensor(df_after["interval"].values,
#                                             # dtype=torch.float32).view(-1,1)
#             # Last observation before the T_val cut_off. THIS IS LIKELY TO GIVE ERRORS IF THE NUMBER OF VALIDATION SAMPLES IS HIGHER THAN 2. CHECK THIS.
#             if batch[0]["store_last"]:
#                 df_last = df.drop_duplicates(subset=["sub_ID"], keep="last").copy() # 对于每个sub_ID,保留它的最后一条记录
#                 index_last = idx2batch[df_last["sub_ID"].values].values # 得到每条记录分别属于哪个样本
...
#         res = {}
#         res["pat_idx"]  = pat_idx # 每个ID下的sub_ID的编号
#         res["times"]    = times # 每个ID下所有sub_ID合并后的所有时间戳
#         res["time_ptr"] = time_ptr # 每个时间戳是从合并后的第几行开始的
#         res["X"]        = torch.tensor(df.iloc[:, value_cols].values)
#         res["M"]        = torch.tensor(df.iloc[:, mask_cols].values)
#         res["obs_idx"]  = torch.tensor(batch_ids) # 按时间戳排序后，每一行对应的是第几个sub_ID
#         res["y"]        = labels # 每一条sub_ID一个labels
#         res["cov"]      = df_cov # 每一条sub_ID一个cov
#         # res["interval"] = interval_tensor 
        
#         res["X_val"]    = X_val
#         res["M_val"]    = M_val
#         res["times_val"]= times_val # 验证的时间戳，（全局拼起来的一维时间轴）
#         res["index_val"]= index_val # 和obs_idx同理，说明每条记录属于哪个样本
#         res["X_last"]   = tens_last
#         res["obs_idx_last"]= index_last
#         # res["interval_val"] = interval_val_tensor
#         res_list.append(res)
