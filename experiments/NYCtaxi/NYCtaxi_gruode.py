...
def train_gruode(simulation_name,params_dict,device, train_idx, val_idx, test_idx, epoch_max=40):
    ...
    dl   = DataLoader(dataset=data_train, collate_fn=data_utils.custom_collate_fn, shuffle=True, batch_size=800,num_workers=2)
        ...
        with torch.no_grad():
            ...
            for i, b in enumerate(dl_val):
                ...
                if params_dict["lambda"]==0:
                    ...
                    value0_mean = 331.1738
                    value0_std = 906.0329
                    value1_mean = 295.6650
                    value1_std = 1075.6835
                    ...
                    epoch_sample_idx += bs
                    if params_dict["lambda"] == 0 and len(epoch_results) > 0:
                        df_epoch = pd.DataFrame(epoch_results)
                        df_epoch.to_csv("val_predictions_raw_scale.csv", index=False)
                        # print(f"[Info] Saved validation predictions (epoch {epoch}) to val_predictions_raw_scale.csv")    
...
if __name__ =="__main__":
    ...
    test_idx = np.load("../../gru_ode_bayes/datasets/NYCtaxi/folds/small_chunk_fold_idx_0/test_idx.npy",allow_pickle=True)

    #Model parameters.
    params_dict=dict()

    params_dict["csv_file_path"] = "../../gru_ode_bayes/datasets/NYCtaxi/NYCtaxi_2017_01-03_irregular_region_1_80_norm.csv" 
